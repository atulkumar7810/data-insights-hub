...
const AboutSection = () => {
  ...
  return (
    <section id="about" className="py-20 md:py-28 lg:py-36 bg-secondary" ref={ref}>
      <motion.div
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center">
          ...
          <motion.div variants={itemLeft}>
            <div className="relative mb-5 md:mb-8">
              <h3 className="font-display text-xl md:text-2xl lg:text-3xl font-bold text-foreground tracking-tight">
                Analyst by Skill, Storyteller by Approach
              </h3>
              <motion.div
                className="mt-3 w-16 h-1 bg-gradient-to-r from-accent to-primary rounded-full"
                initial={{ scaleX: 0 }}
          ...
          <motion.div variants={itemRight} className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-xl lg:max-w-none">
            {highlights.map((item, index) => (
              <motion.div
                ...
                className="card-elevated p-7 rounded-2xl group hover:shadow-xl hover:shadow-accent/10 hover:border-accent/30 transition-all duration-500"
              >
                <motion.div
                  className="w-14 h-14 bg-accent/10 rounded-xl flex items-center justify-center mb-5 group-hover:bg-accent/20 transition-colors"
